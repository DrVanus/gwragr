...
struct NewsArticle: Decodable, Identifiable {
    var id = UUID()  // changed from let to var to remove decode warning
    let title: String
    let link: String
}
...
struct HomeView: View {
    ...
    var body: some View {
        ZStack {
            // Use your dynamic background from Theme.swift
            ...
            NavigationView {
                ScrollView(.vertical, showsIndicators: false) {
                    VStack(alignment: .leading, spacing: 16) {
                        // 1) Portfolio summary / chart near the top
                ...
                .toolbar {
                    // Move settings icon to the right, next to the notifications bell
        ...
        .sheet(isPresented: $showSettings) {
            SettingsView() // Uses your existing SettingsView
        }
        ...
        // If you have an AppTheme, apply color scheme from it:
...
extension HomeView {
    ...
    // New AI Insight block
    private var aiInsightSection: some View {
        VStack(alignment: .leading, spacing: 8) {
            sectionHeading("AI Insight", iconName: "brain.head.profile")
            
            // Example text; you could dynamically update this based on your AI logic
            ...
            // Gold button for deeper analysis
            Button(action: {
                ...
                HStack {
                    NavigationLink(destination: CoinDetailView(coin: coin)) {
                        HStack {
                            ...
                            Text(formatPrice(coin.price))
                                .foregroundColor(.white)
                            // Replace your Text(...) line with either of these approaches:

                            // Approach A: Use String(format:) + an extra '%' at the end:
                            Text("\(String(format: "%.2f", coin.dailyChange))%")

                            // Approach B: Combine the percent symbol right into the format string:
                            Text(String(format: "%.2f%%", coin.dailyChange))
                                .foregroundColor(coin.dailyChange >= 0 ? .green : .red)
                        }
                        .padding(.vertical, 4)
                    ...
                    Button(action: {
                        if let index = marketVM.coins.firstIndex(where: { $0.id == coin.id }) {
                            marketVM.coins[index].isFavorite.toggle()
                        }
                    }) {
